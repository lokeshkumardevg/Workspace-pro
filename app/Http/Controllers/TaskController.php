...
class TaskController extends Controller
{
    public function index(Request $request)
    {
        ...
        $isPrivileged = $user->hasRole(['Super Admin', 'Admin', 'HR', 'Manager', 'super admin', 'admin', 'hr', 'manager', 'Superadmin', 'superadmin']) || $user->hasPermissionTo('view tasks');
    ...
    public function export(Request $request)
    {
        ...
        try {
            if ($request->input('format') === 'doc') {
                $filePath = $this->exportToDoc($tasks, $stats);
            } else {
                $filePath = $this->exportToExcel($tasks, $stats);
            }

            \Illuminate\Support\Facades\Notification::route('mail', 'dev.clientg@gmail.com')
                ->notify(new \App\Notifications\DataExportedNotification($user->name, 'Tasks', $filePath));
    ...
    {
        $filename = "tasks_report_" . date('Y-m-d_His') . ".csv";
    ...
    {
        $filename = "tasks_report_" . date('Y-m-d_His') . ".doc";
        if (!file_exists(storage_path('app/public'))) {
            mkdir(storage_path('app/public'), 0755, true);
        }
        $filePath = storage_path('app/public/' . $filename);
    ...
    public function update(Request $request, Task $task)
    {
        ...
        // Only creator or privileged user can edit
        if (!$isPrivileged && $task->created_by !== $user->id) {
            abort(403, 'Only the creator or admins can edit this task.');
        }
    ...
    public function destroy(Request $request, Task $task)
    {
        ...
        // Only creator or privileged users can delete
        if (!$isPrivileged && $task->created_by !== $user->id) {
        ...
        return redirect()->back()->with('success', '🗑️ Task deleted successfully.');
    }
}
